...
// https://man7.org/linux/man-pages/man3/qsort.3p.html
// https://gist.github.com/jserv/38bca4ebfea1c434e1dfc15337f80eeb
...
again:
    ...
    verify(pthread_mutex_lock(&qs->mtx_st));
    while (qs->st == ts_idle)
        verify(pthread_cond_wait(&qs->cond_st, &qs->mtx_st)); // HHHH: 使用 pthread_cond_wait 等待條件變數激活
    verify(pthread_mutex_unlock(&qs->mtx_st));
    if (qs->st == ts_term) {
        return NULL;
    }
    ...
    if (c->idlethreads == c->nthreads) {
        for (i = 0; i < c->nthreads; i++) {
            ...
            qs2->st = ts_term;
            verify(pthread_cond_signal(&qs2->cond_st)); // JJJJ: 使用 pthread_cond_signal 激活等待狀態的線程
            verify(pthread_mutex_unlock(&qs2->mtx_st));
        }
        verify(pthread_mutex_unlock(&c->mtx_al));
        return NULL;
